...
# General functions used to retrieve values in database based on user entered info
{
  ...
  get_column_associated_with_user_name_func <- function(google_df, user_name_input, user_name_column, selected_column, ...){
    google_df %>% 
  ...
  password_general_func <- function(google_df, user_name_input, user_name_column, selected_column, variable_name){
    get_column_associated_with_user_name_func(google_df, user_name_input, user_name_column, selected_column = variable_name)
  }
}
...
# Forgot password? Enter user_name. If matches, output retrieve question, accept answer
{
  ...
  forgot_password_retrieve_question_func <- function(failed = FALSE, google_df, user_name_input, user_name_column, selected_column) {
    ...
    if(google_df %>% select(!!sym(user_name_column)) %>% filter(. == user_name_input) %>% nrow() != 1){
      ...
      modalDialog(
        ...
        textInput("forgot_password_question_input", 
                  get_column_associated_with_user_name_func(google_df, user_name_input, user_name_column, selected_column),
  ...
  display_password_func <- function(failed = FALSE, google_df, user_name_input, user_name_column, selected_column, variable_name, forgot_password_question_input){
    ### tolower makes case insensitive
    if(tolower(password_general_func(google_df, user_name_input, user_name_column, selected_column, variable_name)) == tolower(forgot_password_question_input)){
      modalDialog(
        h4(paste("Password is:"), get_column_associated_with_user_name_func(google_df, user_name_input, user_name_column, selected_column))
      )
    } else{
      modalDialog(
...
# Reset Password- enter current user name and password, update with new password info
{
  ...
  reset_password_feedback_func <- function(failed = FALSE, google_df, user_name_input, user_name_column, selected_column, existing_password_input){
    ### If user_name doesn't exist, gives error message
    if(google_df %>% select(!!sym(user_name_column)) %>% filter(. == user_name_input) %>% nrow() != 1){
      modalDialog(
        h4("User name doesn't exist or invalid. Retry password reset")
      )
    } else if (get_column_associated_with_user_name_func(google_df, user_name_input, user_name_column, selected_column) == existing_password_input){
      # User name exists prompts retrieve question and response
      modalDialog(
        h4("Reset Password"),
  ...
  password_reset_commit_func <- function(new_password_input, user_name_input, googlesheet_url){
    ...
    # edit google sheet, don't edit with example
    # column with password is "D"
    # range_write(
    #   ss = googlesheet_url, 
    #   data = edit_df, 
    #   range = paste0("D", google_df %>% select(customer_email) %>% filter(. == user_name_input) %>% row_number() +1), 
  ...
  password_updated_func <- function(failed = FALSE){
    modalDialog(
      h4("Password changed. Reload app for password to update.")
    )
  }
...
# Reset Password Retrieve Question and Answer
{
  ...
  reset_password_retrieve_feedback_func <- function(failed = FALSE, google_df, user_name_input, user_name_column, selected_column, existing_password_input){
  ...
  password_retrieve_reset_commit_func <- function(new_password_retrieve_question_input, new_password_retrieve_answer_input,
    ...
    # don't edit sheet in example
    # range_write(
    #   ss = googlesheet_url, 
    #   data = edit_df, 
    #   range = paste0("E", google_df %>% select(customer_email) %>% filter(. == user_name_input) %>% row_number() +1), 
    #   col_names = FALSE)
  }
  ...
  password_retrieve_updated_func <- function(failed = FALSE){
    modalDialog(
      h4("Password retrieve question and answer changed. Reload app for change to be updated.")
    )
  }
}
